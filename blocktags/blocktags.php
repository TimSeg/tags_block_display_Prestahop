	...
	*/
	function hookDisplayHome($params)
	{
		if (!$this->isCached('blocktags.tpl', $this->getCacheId('blocktags')))
		{
			...
			$tags = Db::getInstance()->executeS('SELECT * FROM psst_tag WHERE id_tag= 2 OR id_tag= 1 OR id_tag= 3 OR id_tag= 6 OR id_tag= 7 OR id_tag= 8');
			$times = 0;			
			$max = -1;
			$min = -1;
			foreach ($tags as $tag)
			...
			if (Configuration::get('BLOCKTAGS_RANDOMIZE'))
				shuffle($tags);
			foreach ($tags as &$tag)
				$tag['class'] = 'tag_level'.(int)(($times - $min) * $coef + 1);
